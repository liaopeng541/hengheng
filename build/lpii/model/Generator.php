...
class Generator extends \yii\gii\Generator
{
    ...
    public function getatt($table_name)
    {
        $db = $this->getDbConnection();
        $tableSchema = $db->getTableSchema($table_name);
        $lables = $this->generateLabels($tableSchema);
    ...
    public function generateLabels($table)
    {
        ...
        if($this->getshow())
        {
            $newlable=[];
            $show=$this->getshow();
            foreach ($show['table'] as $key=>$value)
            {
                $newlable[$key]=$value['show_name'];
                $labels=$newlable;
            }
        }
        return $labels;
