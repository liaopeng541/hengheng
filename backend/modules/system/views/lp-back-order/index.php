    ...
    <div class="lp-table-top">
        <div class="layui-btn-group">
            <button class="layui-btn layui-btn-sm layui-btn-danger lp-all-del"
                    lp-data="{table:'lp-back-order',key:'id',url:'<?=\yii\helpers\Url::toRoute("lp-back-order/delete")?>'}"
        ...
        <form class="layui-form lp-search-form" action="#">
            ...
            <div class="layui-input-inline">
                <button lay-submit lp-table="lp-back-order" lay-filter="search" class="layui-btn layui-btn-sm layui-btn-default"><i class="layui-icon">&#xe615;</i>搜索
                </button>
                <div lp-reset-mini-search lp-table="lp-back-order" class="layui-btn layui-btn-sm layui-btn-primary"><i class="layui-icon">&#xe639;</i>重置
                </div>
            </div>
        </form>
    </div>
    <div class="c-c-box">
        <table class="layui-table" lay-filter="lp-back-order-filter"
               lay-data="{cellMinWidth: 80,height:'full-40', page: true,sort:'server', limit:30, url:'<?=\yii\helpers\Url::toRoute("lp-back-order/index")?>','id':'lp-back-order'}">
            <thead>
            <tr>
                <th lay-data="{type:'checkbox'}"></th>
                <th lay-data="{field:'id',sort: true,width:80,align:'center'}">ID</th>
                <th lay-data="{field:'order_sn'}">订单编号</th>
                <th lay-data="{field:'reason'}">退货原因</th>
...
<script type="text/html" id="barDemo">
    <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("lp-back-order/detail")?>',key:'id',type:2,maxmin:true,area: ['550px', '500px'],title:'详情'}"
