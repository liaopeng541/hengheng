...
<div class="lp-grid">
    ...
    <div class="c-c-box">
        <table class="layui-table" lay-filter="lp-user-level-filter" lay-size="sm" lay-data="{cellMinWidth: 80,height:'full-40', page: true,sort:'server', limit:30, url:'<?=\yii\helpers\Url::toRoute("lp-user-level/index")?>','id':'lp-user-level'}">
            <thead>
            <tr>
...
<script src="admin/src/layui.js"></script>
            ...
            <script type="text/html" id="list_showTpl">
                <div class="lp-col-center ">
                    <input type="checkbox" lp-data="{url:'<?=\yii\helpers\Url::toRoute("lp-user-level/setstatus")?>',id:{{d.level_id}},on:'1',off:'0',key:'level_id',table:'lp-user-level'}" name="list_show" lay-skin="switch" lay-filter="lp-user-level"  title="开关" lay-text="启用|禁用">
                </div>
            </script>

        
    <script type="text/html" id="barDemo">
                    <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("lp-user-level/detail")?>',key:'level_id',type:2,maxmin:true,area: ['893px', '600px'],title:'详情'}"  class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail">
                <i class="layui-icon">&#xe60a;</i> 查看
            </a>
                            <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("lp-user-level/update")?>',key:'level_id',type:2,maxmin:true,area: ['893px', '600px'],title:'编辑'}" class="layui-btn layui-btn-xs" lay-event="edit"><i class="layui-icon">&#xe642;</i>编辑
            </a>
                            <a lp-data="{table:'lp-user-level',key:'level_id',url:'<?=\yii\helpers\Url::toRoute("lp-user-level/delete")?>'}" class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del"><i class="layui-icon">&#xe640;</i>删除
            </a>
            </script>
