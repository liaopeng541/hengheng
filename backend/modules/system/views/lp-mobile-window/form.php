...
<div class="container-fluid " style="padding:10px">
    <form class="layui-form layui-form-pane" action="<? if(isset($_GET["id"])){ echo \yii\helpers\Url::toRoute("lp-mobile-window/update");}else{echo \yii\helpers\Url::toRoute("lp-mobile-window/add");}?>" lp-data="{<?if(isset($_GET["id"])){?>type:'edit',key:'id',value:<?=$_GET["id"]?>,<?}?>table:'lp-mobile-window'}">
        <div class="layui-form-item">
                            <label class="layui-form-label">名字</label>
                            <div class="layui-input-block">
                                <input class="layui-input" name="name" required lay-verify="required" lay-error="名字不能为空" placeholder="请输入名字" />
                            </div>
                        </div>
                                <div class="layui-form-item">
