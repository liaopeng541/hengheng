...
<div class="lp-grid">
    ...
    <div class="c-c-box">
        <table class="layui-table" lay-filter="hh-store-filter" lay-size="sm" lay-data="{cellMinWidth: 80,height:'full-40', page: true,sort:'server', limit:30, url:'<?=\yii\helpers\Url::toRoute("hh-store/index")?>','id':'hh-store'}">
            <thead>
            <tr>
...
<script src="admin/src/layui.js"></script>
            ...
            <script type="text/html" id="statusTpl">
                <div class="lp-col-center ">
                    <input type="checkbox" lp-data="{url:'<?=\yii\helpers\Url::toRoute("hh-store/setstatus")?>',id:{{d.id}},on:'1',off:'0',key:'id',table:'hh-store'}" name="status" lay-skin="switch" lay-filter="hh-store"  title="开关" lay-text="启用|禁用">
                </div>
            </script>

        
    <script type="text/html" id="barDemo">
                    <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-store/detail")?>',key:'id',type:2,maxmin:true,area: ['893px', '600px'],title:'详情'}"  class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail">
                <i class="layui-icon">&#xe60a;</i> 查看
            </a>
                            <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-store/update")?>',key:'id',type:2,maxmin:true,area: ['893px', '600px'],title:'编辑',full:true}" class="layui-btn layui-btn-xs" lay-event="edit"><i class="layui-icon">&#xe642;</i>编辑
            </a>
                            <a lp-data="{table:'hh-store',key:'id',url:'<?=\yii\helpers\Url::toRoute("hh-store/delete")?>'}" class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del"><i class="layui-icon">&#xe640;</i>删除
            </a>
            </script>
