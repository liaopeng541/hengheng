...
<div class="container-fluid " style="padding:10px">
    <form class="layui-form layui-form-pane" action="<? if(isset($_GET["id"])){ echo \yii\helpers\Url::toRoute("hh-live/update");}else{echo \yii\helpers\Url::toRoute("hh-live/add");}?>" lp-data="{<?if(isset($_GET["id"])){?>type:'edit',key:'id',value:<?=$_GET["id"]?>,<?}?>table:'hh-live'}">
        <div class="layui-form-item">
                            <label class="layui-form-label">摄像头</label>
                            <div class="layui-input-block">
                                <input class="layui-input" name="crame" required lay-verify="required" lay-error="摄像头不能为空" placeholder="请输入摄像头" />
                            </div>
                        </div>
                        <div class="layui-form-item">
                            <label class="layui-form-label">设备号</label>
                            <div class="layui-input-block">
                                <input class="layui-input" name="device" required lay-verify="required" lay-error="设备号不能为空" placeholder="请输入设备号" />
                            </div>
                        </div>
                        <div class="layui-form-item">
                            <label class="layui-form-label">工位</label>
                            <div class="layui-input-block">
                                <select name="work_id" required lay-verify="required" lay-error="工位不能为空" placeholder="请输入工位">
