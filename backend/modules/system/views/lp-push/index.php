...
<div class="lp-grid">
    <div class="lp-table-top">
        <div class="layui-btn-group">
            <button class="layui-btn layui-btn-sm lp-modal" lp-data="{content:'<?=\yii\helpers\Url::toRoute("lp-push/add")?>',type:2,maxmin:true,area: ['600px', '500px'],title:'添加' }" style="background: #2980B9"><i class="layui-icon"></i>添加
            </button>
            <button class="layui-btn layui-btn-sm layui-btn-danger lp-all-del"
                    lp-data="{table:'lp-push',key:'id',url:'<?=\yii\helpers\Url::toRoute("lp-push/delete")?>'}"
    ...
    <div class="c-c-box">
        <table class="layui-table" lay-filter="lp-push-filter"
               lay-data="{cellMinWidth: 80,height:'full-40', page: true,sort:'server', limit:30, url:'<?=\yii\helpers\Url::toRoute("lp-push/index")?>','id':'lp-push'}">
            <thead>
            <tr>
                ...
                <th lay-data="{fixed: 'right',  align:'center',width:280, toolbar: '#barDemo'}"></th>
            </tr>
            </thead>
        </table>
...
<script type="text/html" id="thumbTpl">
    <div class="lp-col-center ">
        <div {{# if(d.thumb){}}class="lp-zoom" {{# }}}>
            <img src="{{# if(d.thumb){}}{{d.thumb}}{{# }else{}}admin/images/avatar.png{{# }}}" jqimg="{{# if(d.thumb){}}{{d.thumb}}{{# }else{}}admin/images/avatar.png{{# }}}"
...
<script type="text/html" id="barDemo">
    <a class="layui-btn layui-btn-primary layui-btn-xs layui-btn-jPush" lp-data="{{ d.id }}">
        <i class="layui-icon">&#xe60a;</i> 推送
    </a>
    <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("lp-push/detail")?>',key:'id',type:2,maxmin:true,area: ['600px', '500px'],title:'详情'}"
       class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail">
        <i class="layui-icon">&#xe60a;</i> 查看
    </a>
    <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("lp-push/update")?>',key:'id',type:2,maxmin:true,area: ['600px', '500px'],title:'编辑'}"
       class="layui-btn layui-btn-xs" lay-event="edit"><i class="layui-icon">&#xe642;</i>编辑
    </a>
    <a lp-data="{table:'lp-push',key:'id',url:'<?=\yii\helpers\Url::toRoute("lp-push/delete")?>'}"
...
<script>
    layui.use(['lpsearchbox', 'jquery', 'element', 'lpui', 'lpzoom', 'lplist', 'lpform'], function(){
        var $ = layui.jquery;
        var jPushUrl = "<?=\yii\helpers\Url::toRoute("message-push/push")?>";
        $(document).on('click','.layui-btn-jPush',function() {
            var mid = $(this).attr('lp-data');
            if (mid > 0) {
                $.getJSON(jPushUrl, {'id':mid}, function(res) {
                    layer.msg(res.msg);
                });
            } else {
                return false;
