    ...
    <div class="lp-table-top">
        <div class="layui-btn-group">
            <button class="layui-btn layui-btn-sm lp-modal"
                        lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-worker/add")?>',type:2,maxmin:true,area: ['600px', '600px'],title:'添加' }"
                        style="background: #2980B9"><i class="layui-icon"></i>添加
                </button>
                                        <button class="layui-btn layui-btn-sm layui-btn-danger lp-all-del"
                        lp-data="{table:'hh-worker',key:'id',url:'<?=\yii\helpers\Url::toRoute("hh-worker/delete")?>'}"
...
<option value="1">忙碌</option>
    ...
    <div class="c-c-box">
        <table class="layui-table" lay-filter="hh-worker-filter"
               lay-data="{cellMinWidth: 80,height:'full-40', page: true,sort:'server', limit:30, url:'<?=\yii\helpers\Url::toRoute("hh-worker/index")?>','id':'hh-worker'}">
            <thead>
            <tr>
...
<script src="admin/src/layui.js"></script>
            ...
            <script type="text/html" id="header_picTpl">
                <div class="lp-col-center ">
                    <div {{# if(d.header_pic){}}class="lp-zoom" {{# }}}>
                        <img src="{{# if(d.header_pic){}}{{d.header_pic}}{{# }else{}}admin/images/avatar.png{{# }}}"
                             jqimg="{{# if(d.header_pic){}}{{d.header_pic}}{{# }else{}}admin/images/avatar.png{{# }}}"
                             {{# if(d.header_pic){}} class="lp-img" {{# }}}>
                    </div>
                </div>
            </script>
                ...
                <div class="lp-col-detail" lay-event="show_detail"
                     lp-data="{type:2,content:'<?=\yii\helpers\Url::toRoute("hh-branch/detail")?>',area: ['893px', '600px'],title:'详情',key:'branch_id'}">
                    {{ d.branch_id_lp }}
                </div>
            </script>
                ...
                <div class="lp-col-detail" lay-event="show_detail"
                     lp-data="{type:2,content:'<?=\yii\helpers\Url::toRoute("lp-job/detail")?>',area: ['893px', '600px'],title:'详情',key:'job_id'}">
                    {{ d.job_id_lp }}
                </div>
            </script>
    ...
    <script type="text/html" id="barDemo">
                    <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-worker/detail")?>',key:'id',type:2,maxmin:true,area: ['600px', '600px'],title:'详情'}"
               class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail">
                <i class="layui-icon">&#xe60a;</i> 查看
            </a>
                            <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-worker/update")?>',key:'id',type:2,maxmin:true,area: ['600px', '600px'],title:'编辑'}"
               class="layui-btn layui-btn-xs" lay-event="edit"><i class="layui-icon">&#xe642;</i>编辑
            </a>
                            <a lp-data="{table:'hh-worker',key:'id',url:'<?=\yii\helpers\Url::toRoute("hh-worker/delete")?>'}"
