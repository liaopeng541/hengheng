...
<div class="lp-grid">
    <div class="lp-table-top">
        <div class="layui-btn-group">
                        ...
                        lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-car/add")?>',type:2,maxmin:true,area: ['893px', '600px'],title:'添加' }"
                        style="background: #2980B9"><i class="layui-icon"></i>添加
                </button>
                                        <button class="layui-btn layui-btn-sm layui-btn-danger lp-all-del"
                        lp-data="{table:'hh-car',key:'id',url:'<?=\yii\helpers\Url::toRoute("hh-car/delete")?>'}"
    ...
    <div class="c-c-box">
        <table class="layui-table" lay-filter="hh-car-filter"
               lay-data="{cellMinWidth: 80,height:'full-40', page: true,sort:'server', limit:30, url:'<?=\yii\helpers\Url::toRoute("hh-car/index")?>','id':'hh-car'}">
            <thead>
            <tr>
                ...
                <th lay-data="{field:'brand_logo'}">Brand Logo</th>
...
<script type="text/html" id="brand_logoTpl">
    <div class="lp-col-center ">
        <div {{# if(d.brand_logo){}}class="lp-zoom" {{# }}}>
            <img src="{{# if(d.brand_logo){}}{{d.brand_logo}}{{# }else{}}admin/images/avatar.png{{# }}}"
                 jqimg="{{# if(d.brand_logo){}}{{d.brand_logo}}{{# }else{}}admin/images/avatar.png{{# }}}"
                 {{# if(d.brand_logo){}} class="lp-img" {{# }}}>
        </div>
    </div>
</script>
        
<script type="text/html" id="barDemo">
    <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-car/detail")?>',key:'id',type:2,maxmin:true,area: ['893px', '600px'],title:'详情'}"
       class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail">
        <i class="layui-icon">&#xe60a;</i> 查看
    </a>
                    <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-car/update")?>',key:'id',type:2,maxmin:true,area: ['893px', '600px'],title:'编辑'}"
       class="layui-btn layui-btn-xs" lay-event="edit"><i class="layui-icon">&#xe642;</i>编辑
    </a>
                    <a lp-data="{table:'hh-car',key:'id',url:'<?=\yii\helpers\Url::toRoute("hh-car/delete")?>'}"
