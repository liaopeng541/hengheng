...
<div class="lp-grid">
    <div class="lp-table-top">
        <div class="layui-btn-group">
            <button class="layui-btn layui-btn-sm lp-modal"
                    lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-work-station/add")?>',type:2,maxmin:true,area: ['600px', '450px'],title:'添加' }"
                    style="background: #2980B9"><i class="layui-icon"></i>添加
            </button>
            <button class="layui-btn layui-btn-sm layui-btn-danger lp-all-del"
                        lp-data="{table:'hh-work-station',key:'id',url:'<?=\yii\helpers\Url::toRoute("hh-work-station/delete")?>'}"
        ...
        <form class="layui-form lp-search-form" action="#">
            ...
            <div class="layui-input-inline">
                <button lay-submit lp-table="hh-work-station" lay-filter="search" class="layui-btn layui-btn-sm layui-btn-default"><i class="layui-icon">&#xe615;</i>搜索
                </button>
                <div lp-reset-mini-search lp-table="hh-work-station" class="layui-btn layui-btn-sm layui-btn-primary"><i class="layui-icon">&#xe639;</i>重置
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="c-c-box">
        <table class="layui-table" lay-filter="hh-work-station-filter"
               lay-data="{cellMinWidth: 80,height:'full-40', page: true,sort:'server', limit:30, url:'<?=\yii\helpers\Url::toRoute("hh-work-station/index")?>','id':'hh-work-station'}">
            <thead>
            <tr>
...
<script type="text/html" id="service_idTpl">
    <div class="lp-col-center" lp-data="{type:2,content:'<?=\yii\helpers\Url::toRoute("hh-service/detail")?>',area: ['893px', '600px'],title:'详情',key:'service_id'}">
        {{ d.service_id_lp }}
    </div>
</script>
...
<script type="text/html" id="statusTpl">
    <div class="lp-col-center ">
        <input type="checkbox"
               lp-data="{url:'<?=\yii\helpers\Url::toRoute("hh-work-station/setstatus")?>',id:{{d.id}},on:'1',off:'0',key:'id',table:'hh-work-station'}"
               name="status" lay-skin="switch" lay-filter="hh-work-station"
               title="开关" lay-text="启用|禁用" {{# if(d.status==1){ }} checked {{# }}}>
    </div>
</script>

<script type="text/html" id="barDemo">
    <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-work-station/detail")?>',key:'id',type:2,maxmin:true,area: ['600px', '450px'],title:'详情'}"
           class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail"><i class="layui-icon">&#xe60a;</i> 查看
    </a>
    <a lp-data="{content:'<?=\yii\helpers\Url::toRoute("hh-work-station/update")?>',key:'id',type:2,maxmin:true,area: ['600px', '450px'],title:'编辑'}" class="layui-btn layui-btn-xs" lay-event="edit"><i class="layui-icon">&#xe642;</i>编辑
    </a>
