...
<div class="container-fluid " style="padding:10px">
    <form class="layui-form layui-form-pane" action="<? if(isset($_GET["id"])){ echo \yii\helpers\Url::toRoute("hh-process-cat/update");}else{echo \yii\helpers\Url::toRoute("hh-process-cat/add");}?>" lp-data="{<?if(isset($_GET["id"])){?>type:'edit',key:'id',value:<?=$_GET["id"]?>,<?}?>table:'hh-process-cat'}">
        <div class="layui-form-item">
                            <label class="layui-form-label">上级分类</label>
                            <div class="layui-input-block">
                                <select name="pid" required lay-verify="required" lay-error="上级分类不能为空" placeholder="请输入上级分类">
                                    <option value="0">顶级上级分类</option>
