...
<div class="container-fluid " style="padding:10px">
    <form class="layui-form layui-form-pane" action="<? if(isset($_GET["id"])){ echo \yii\helpers\Url::toRoute("hh-oto-order-cancel-cause/update");}else{echo \yii\helpers\Url::toRoute("hh-oto-order-cancel-cause/add");}?>" lp-data="{<?if(isset($_GET["id"])){?>type:'edit',key:'id',value:<?=$_GET["id"]?>,<?}?>table:'hh-oto-order-cancel-cause'}">
        <div class="layui-form-item">
                            <label class="layui-form-label">退单原因</label>
                            <div class="layui-input-block">
                                <input class="layui-input" name="cause" required lay-verify="required" lay-error="退单原因不能为空" placeholder="请输入退单原因" />
                            </div>
                        </div>
                                <div class="layui-form-item">
