...
layui.define(['form', 'layer', 'jqelem', 'jqajax'], function(exports) {
        ...
        for (var key in record) {
            ...
            if (obj.length > 0) {
                if (obj.prop('type') == "text" || obj.prop('type') == "hidden" || obj.prop('type') == "password") {
                    ...
                    if (theForm.find(_this.config.imgsClass + '[name=' + key + ']').length > 0) {
                        ...
                        theForm.find(_this.config.imgsClass + '[name=' + key + ']').parents("div.layui-input-block").after(html);}
                    }
    ...
    Jqform.prototype.check = function(obj, isTab) {
        var ver = obj.attr('jq-verify'),
            ...
            obj.parent('div').removeClass(DANGER);
            obj.parent('div').children(".jq-error").remove();
