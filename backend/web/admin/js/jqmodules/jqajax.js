...
layui.define(['jquery', 'layer'], function(exports) {
    ...
    jqajax.prototype.ajax = function(options) {
        ...
        $.ajax({
            ...
            success: function(msg) {
                if(msg.ok)
                {
                    $("#wx").html(parseFloat(msg.ok.wx).toFixed(2));
                    $("#ali").html(parseFloat(msg.ok.ali).toFixed(2));
                    $("#xj").html(parseFloat(msg.ok.xj).toFixed(2));
                    $("#fd").html(parseFloat(msg.ok.fd).toFixed(2));
                    $("#wf").html(parseFloat(msg.ok.wf).toFixed(2));


                }
                if(msg.atta)
                {
                    for(var item in msg.atta)
                    {
                        $("#"+item).val(msg.atta[item]);
                    }
                }
    ...
    jqajax.prototype.complete = function(ret, options) {
        ...
        if (200 == ret.status) {
            ...
            if (options.action == "edit") {
                ...
                {
                    var table = top.global[options.dataName]
                    var data=table.options.list.list;
                    for(var i = 0; i<ret.data.length; i++)
                    {
                        for(var j=0;j<data.length;j++)
                        {
                            if(ret.data[i][options.key]==data[j][options.key])
                            {
                                top.global[options.dataName].options.list.list[j]=ret.data[i];
                            }
                        }
                    }
                }
            ...
            if(ret.menu)
            {
                ...
                menu_list.empty();
                menu_list.append('<option value="0">顶级菜单</option>')
                for(var i =0;i<ret.menu.length;i++)
                {
                    menu_list.append('<option value="'+ret.menu[i].id+'">'+ret.menu[i].name+'</option>')
                }
            }
        ...
        } else if(8==ret.status) {
            parent.layer.msg(ret.message,{ icon: 5 });
            _this.action(options);
        }else if(9==ret.status) {
            if (self != top) {
                parent.location.href="/admin/web/index.php?r=index/login";
            }else {
                location.href="/admin/web/index.php?r=index/login";
            }
            return
        }else{
            if (index) {
        ...
        if (options.confirm) {
            ...
            if (options.title) {
                ...
                if (obj) {
                    ...
                    title = '您确认要<span class="color-commred">' + title + '</span>吗';
                }
            }
    ...
    jqajax.prototype.action = function(options) {
        ...
        if (typeof(params[options.key]) != "string") {
            ...
            return false;
            */
            params[options.key]=params[options.key].toString();
    ...
    jqajax.prototype.sort = function(params) {
        ...
        if (jqtable.options.server) {
            var lp_jqtable = top.global[params.dataName];
            params.url=jqtable.options.url;
            params.url=params.url.replace(/p=\d+/, "p=1");
            params.url=params.url.replace(/&sort=\S+/, "");
            var lp_str='&';
            if(params.url.indexOf("?")==-1)
            {
                lp_str='?';
            }
            params.url+=lp_str+"sort="+params.field+"|"+params.sort;
            params.action="list"
            params.method="POST";
            this.ajax(params);
        } else {
