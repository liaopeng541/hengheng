...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  Class.prototype.setArea = function(){
    ...
    /**liaopeng修改表格reload造成的滚动条bug**/
    if(options.autoWidthCount){
        that.autoColNums=options.autoWidthCount;
    }else{
        options.autoWidthCount=that.autoColNums = autoColNums; //记录自动列数
    }
    /**修改结束**/
    ...
    layui.each(options.cols, function(i1, item1){
      layui.each(item1, function(i2, item2){
        ...
        if(item2.colspan > 1) return;
        if(item2.width === 0 && !item2.isSetWidth){
  ...
  Class.prototype.pullData = function(curr, loadIndex){
    ...
    that.startTime = new Date().getTime(); //渲染开始时间
      ...
      /**liaopeng修改表格调整大小时使用本地数据**/
      if(options.localreload)
      {
          res=table.lastpage[options.id];
          that.renderData(res, curr, res[response.countName]), sort();
          options.localreload=false;
      ...
      $.ajax({
        ...
        ,success: function(res){
          if(res[response.statusName] != response.statusCode){
            ...
            table.lastpage[options.id]=res;
            table.lastpage[options.id].curr=curr;
            table.lastpage[options.id].where=data;
              table.lastpage[options.id].url=options.url;
            that.renderData(res, curr, res[response.countName]), sort();
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    render();
      ...
      /**liaopeng添加自定义一些事件绑定**/
      $(that.elem).find(".lp-zoom").unbind();
      if(that.timer_event)
      {
        clearTimeout(that.timer_event)
      }
      that.timer_event=setTimeout(function () {
          var settings = {
              xzoom: 200,
              yzoom: 200,
              offset: 10,
          ...
          var scrolly=0;
          var scrollx=0;
          $(that.elem).find(".lp-zoom").hover(function () {
              ...
              var lp_left=$(this).offset().left;
              lp_left+=$(this).width();
              lp_left+=settings.xzoom;
              var ax=$(document.body).width()-30;
              if(ax<lp_left)
              {
                  settings.position="left";
              }
              var lp_top=$(this).offset().top;
              lp_top+=settings.yzoom;
              var ay=$(document.body).height()-60;
              var newtop=imageTop;
              if(ay<lp_top)
              {
                  newtop=ay-settings.yzoom;
              }
              ...
              $("div.zoomdiv").css({ top: newtop, left: leftpos });
          ...
          $(that.elem).find(".lp-zoom").click(function () {
              var img=$(this).find("img").attr("jqimg");
              var content='<div><img src="'+img+'" class="lp-show-img"></div>';
              layui.layer.open({
                  type: 1,
                  title: false,
                  area: '[80%,80%]',
    ...
    if(options.page){
      ...
      if(options.localreload){options.page.curr=table.lastpage[options.id].curr};
      options.page.count = count; //更新总条数
      laypage.render(options.page);
    }
      table.lastpage[options.id].options=options;
  };
  ...
  Class.prototype.sort = function(th, type, pull, formEvent){
    ...
    that.sortKey = {
      ...
      /**liaopeng修改排序，方便升级时检索，在参数处传入排序方式**/
  ...
  Class.prototype.events = function(){
    ...
    that.layBody.on("click",'tr',function (e) {
        if($(e.target).parent(".layui-form-checkbox").length==0 && $(e.target).parents(".layui-table-fixed").length==0) {

            var othis = $(this)
                , index = othis.data('index')
                , data = table.cache[that.key][index],
                checkbox = othis.find('input[name="layTableCheckbox"]+').prev()
                , checked = checkbox[0].checked
            if (options.rowcheck) {
                checked = !checked
                $(checkbox[0]).prop("checked", checked)
                that.renderForm('checkbox');
                that.setCheckData(index, checked);
                that.syncCheckAll();
            }
    ...
    /**liaopeng修改resize重新init在lplist里**/
    /*
    var timer=0;
    ...
    layui.each(data, function(i, item){
      ...
      if(item[table.config.checkName]){
        ...
        arr.push(table.clearCacheKey(item));
          index.push(i);//liaopeng增加行号的传递
      }
    });
    return {
      data: arr //选中的数据
        ,index:index//liaopeng增加行号的传递
